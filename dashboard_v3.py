...
# ──────────────────────────────────────────────
#  DETECCIÓN DE ISP / HOTSPOT PERSONAL
# ──────────────────────────────────────────────
import re

# OUIs de fabricantes de equipos de operadoras (routers de ISP)
ISP_HARDWARE_OUIS = {
    "00:18:82": "Huawei (ISP)",  "00:1E:10": "Huawei (ISP)",
    "00:25:9E": "Huawei (ISP)",  "04:02:1F": "Huawei (ISP)",
    "14:B9:68": "Huawei (ISP)",  "2C:AB:00": "Huawei (ISP)",
    "4C:1F:CC": "Huawei (ISP)",  "54:89:98": "Huawei (ISP)",
    "64:3E:8C": "Huawei (ISP)",  "78:1D:BA": "Huawei (ISP)",
    "00:19:CB": "ZTE (ISP)",     "00:22:93": "ZTE (ISP)",
    "2C:26:C5": "ZTE (ISP)",     "34:4B:50": "ZTE (ISP)",
    "50:6E:AA": "ZTE (ISP)",     "64:13:6C": "ZTE (ISP)",
    "A0:AB:1B": "ZTE (ISP)",     "BC:76:70": "ZTE (ISP)",
    "F4:6D:04": "ZTE (ISP)",     "FC:8B:97": "ZTE (ISP)",
    "00:16:76": "Sagemcom",      "00:90:D0": "Sagemcom",
    "00:08:9F": "Technicolor",   "00:19:15": "Technicolor",
    "10:C3:7B": "Arcadyan",      "38:2C:4A": "Arcadyan",
    "84:9F:B5": "Nokia (ISP)",   "00:09:B0": "Nokia (ISP)",
    "00:11:F5": "Ericsson (ISP)","00:19:2F": "Ericsson (ISP)",
}

# Patrones de SSID por operadora (regex, case-insensitive)
ISP_SSID_PATTERNS = [
    # ── Perú ───────────────────────────────────
    (r"claro[-_\s]?\w*",                  "Claro (Perú)"),
    (r"claro\s?hogar",                    "Claro Hogar (Perú)"),
    (r"movistar[-_\s]?\w*",               "Movistar (Perú)"),
    (r"entel[-_\s]?\w*",                  "Entel (Perú)"),
    (r"bitel[-_\s]?\w*",                  "Bitel (Perú)"),
    (r"tigo[-_\s]?\w*",                   "Tigo (Perú)"),
    (r"wom[-_\s]?\w*",                    "WOM (Perú)"),
    (r"win[-_\s]?\w*",                    "Win (Perú)"),
    (r"optical\s?networks",               "Optical Networks (Perú)"),
    (r"americatel",                       "Americatel (Perú)"),
    # ── Otros países región ────────────────────
    (r"izzi[-_\s]?\w*",                   "Izzi (México)"),
    (r"telmex[-_\s]?\w*",                 "Telmex (México)"),
    (r"totalplay",                        "TotalPlay (México)"),
    (r"megacable",                        "Megacable (México)"),
    (r"une[-_\s]?\w*",                    "UNE (Colombia)"),
    (r"etb[-_\s]?\w*",                    "ETB (Colombia)"),
    (r"directv",                          "DirecTV"),
    (r"speedy",                           "Speedy"),
    (r"fibertel",                         "Fibertel"),
    (r"personal[-_\s]?\w*",              "Personal"),
    # ── Genéricos router ISP ───────────────────
    (r"^home[-_]?\d+",                    "Router doméstico"),
    (r"^TPLINK[-_]?\w*",                  "TP-Link"),
    (r"^HUAWEI[-_]?\w*",                  "Router Huawei"),
    (r"^ZTE[-_]?\w*",                     "Router ZTE"),
    (r"^ARRIS[-_]?\w*",                   "Router ARRIS"),
]

# Patrones de HOTSPOT PERSONAL (esto es lo más crítico para exámenes)
HOTSPOT_PATTERNS = [
    # ── iOS (mantener) ──────────────────────────
    (r"iphone\s+de\s+\w+",              "📱 Hotspot iPhone"),
    (r"ipad\s+de\s+\w+",                "📱 Hotspot iPad"),
    (r"iphone\s+\w+",                   "📱 Hotspot iPhone"),
    (r"^iphone$",                       "📱 Hotspot iPhone"),
    # ── Android genérico (mantener) ─────────────
    (r"androidap",                      "📱 Hotspot Android"),
    (r"android\s*ap",                   "📱 Hotspot Android"),
    # ── Marcas de móviles (nueva versión flexible) ──
    (r"\b(?:galaxy|samsung)\b",         "📱 Hotspot Samsung Galaxy"),
    (r"\b(?:redmi|xiaomi|poco)\b",      "📱 Hotspot Xiaomi/Redmi/POCO"),
    (r"\bhuawei\b",                     "📱 Hotspot Huawei"),
    (r"\bhonor\b",                      "📱 Hotspot Honor"),   # <-- SOLUCIÓN
    (r"\bmoto\w*\b",                    "📱 Hotspot Motorola"),
    (r"\bpixel\b",                      "📱 Hotspot Google Pixel"),
    (r"\boneplus\b",                    "📱 Hotspot OnePlus"),
    (r"\blg\b",                         "📱 Hotspot LG"),
    (r"\btecno\b",                      "📱 Hotspot Tecno"),
    (r"\binfinix\b",                    "📱 Hotspot Infinix"),
    (r"\b(?:oppo|vivo|realme|nokia|sony)\b", "📱 Hotspot Otra marca"),
    # ── Windows / Hotspot genérico ──────────────
    (r"hotspot\s+de\s+\w+",             "💻 Hotspot Windows/Android"),
    (r"zona\s+wi\.?fi",                 "📱 Hotspot personal"),
    (r"punto\s+de\s+acceso",            "📱 Hotspot personal"),
    (r"punto\s+acceso",                 "📱 Hotspot personal"),
    # ── Patrones especiales ─────────────────────
    (r"^DIRECT-\w+",                    "📱 Hotspot directo (Wi-Fi Direct)"),
    (r"^AndroidShare_\w+",              "📱 Hotspot Android compartido"),
    # ── Nombres comunes adicionales ─────────────
    (r"\b(cel|celu|celi|cels)\b",       "📱 Probable hotspot celular"),
    (r"wifi\s*(cel|movil|phone|fono)",  "📱 Probable hotspot celular"),
]

def analyze_ssid(ssid: str) -> dict:
    """
    Analiza el SSID para detectar:
    - Si pertenece a un ISP conocido
    - Si es un hotspot personal (MUY relevante para exámenes)
    Retorna dict con: is_hotspot, is_isp, label, alert_level
    """
    if not ssid or ssid in ("[Oculto]", "[Hidden]", "Hidden"):
        return {"is_hotspot": False, "is_isp": False, "label": "", "alert_level": "NONE"}

    ssid_lower = ssid.lower().strip()

    # Verificar hotspot personal PRIMERO (mayor prioridad)
    for pattern, label in HOTSPOT_PATTERNS:
        if re.search(pattern, ssid_lower, re.IGNORECASE):
            return {
                "is_hotspot": True,
                "is_isp":     False,
                "label":      label,
                "alert_level": "CRITICAL",  # Hotspot = alerta máxima en examen
                "detail":     f"HOTSPOT PERSONAL detectado: '{ssid}'. Indica un celular activo compartiendo internet, lo cual es altamente sospechoso en un examen."
            }

    # Verificar ISP
    for pattern, isp_name in ISP_SSID_PATTERNS:
        if re.search(pattern, ssid_lower, re.IGNORECASE):
            return {
                "is_hotspot": False,
                "is_isp":     True,
                "label":      f"🌐 {isp_name}",
                "alert_level": "INFO",
                "detail":     f"Red del proveedor '{isp_name}'. Probablemente un router fijo del entorno, no un celular."
            }

    return {"is_hotspot": False, "is_isp": False, "label": "", "alert_level": "NONE"}


def check_oui_isp(mac: str) -> str:
    """Verifica si el OUI pertenece a hardware de ISP."""
    mac_up = mac.upper().replace("-",":")
    oui = mac_up[:8] if len(mac_up) >= 8 else ""
    return ISP_HARDWARE_OUIS.get(oui, "")
...
C_HOTSPOT   = "#FF3E3E"   # Rojo intenso para hotspot personal (máximo riesgo)
C_ISP       = "#4FC3F7"   # Azul claro para redes de ISP conocido
...
def enrich_device(d: dict) -> dict:
    ...
    # Análisis de SSID: hotspot / ISP
    ssid_info = analyze_ssid(ssid)

    # OUI de hardware ISP como respaldo
    isp_hw = check_oui_isp(mac)
    if isp_hw and not ssid_info["is_isp"] and not ssid_info["is_hotspot"]:
        ssid_info["is_isp"]  = True
        ssid_info["label"]   = f"🌐 {isp_hw}"
        ssid_info["detail"]  = f"OUI corresponde a equipamiento de operadora ({isp_hw})."
    ...
    d["is_hotspot"]    = ssid_info.get("is_hotspot", False)
    d["is_isp"]        = ssid_info.get("is_isp", False)
    d["ssid_label"]    = ssid_info.get("label", "")
    d["ssid_detail"]   = ssid_info.get("detail", "")
...
class DeviceDetailWindow:
    ...
    def _tab_signal(self, parent, d):
        ...
        progress = ctk.CTkProgressBar(bar_frame, width=400, height=16,
                              fg_color=C_BORDER, progress_color=bar_color)
        progress.pack(padx=16, pady=(0,4))
...
class EMFDashboard:
    ...
    def _update_table(self, devices, crit, warn, rands, hids):
        ...
        for d in sorted_devs:
            ...
            if d.get("is_isp"):      flags.append("📡ISP")      # NUEVO
            if d.get("is_hotspot"):  flags.append("🔥HOT")     # NUEVO
